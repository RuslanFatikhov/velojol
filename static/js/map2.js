document.addEventListener('DOMContentLoaded', function() {
    ...
    function createPopUpHtml(bikeLane) {
        let photosHtml = bikeLane.photos?.map(photo => `<img src="${photo}" data-imageview alt="Фото велодорожки" style="max-width: 100px; margin-right: 5px;">`).join('') || '';
    ...
    function handleBikeLaneClick(bikeLane) {
        ...
        customPopup.innerHTML = createPopUpHtml(bikeLane);
        customPopup.style.display = 'block';
        initImageView(); // Вызываем для инициализации обработчиков событий на новых изображениях
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var map = new mapboxgl.Map({
        container: 'map',
        style: 'mapbox://styles/mapbox/dark-v11',
        center: cityCoordinates, // Замените cityCoordinates на реальные координаты центра карты
        zoom: 11
    });
    ...
    function createPopUpHtml(bikeLane) {
        ...
        if (typeof bikeLane.safetyLevel === 'number') {
            ratingLabel = ['Ужасно', 'Плохо', 'Удовлетворительно', 'Хорошо', 'Отлично'][bikeLane.safetyLevel - 1] || ratingLabel;
        }
