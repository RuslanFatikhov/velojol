...
map.on('load', () => {
    ...
    fetch('bikePathsData-eng.json')
        .then(response => response.json())
        .then(data => {
            data.forEach(bikePath => {
                const safetyLevels = {
                    5: { color: '#64C750', text: 'Great' },
                    4: { color: '#FFBD3F', text: 'Good' },
                    3: { color: '#E55D47', text: 'Bad' },
                    2: { color: '#772613', text: 'Badly' },
                    1: { color: '#121212', text: 'Terrible' }
                };
